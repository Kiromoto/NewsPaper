...
def my_job():
    print('Mail_send start!')
    send_mail(subject='job mail',
              message='Hello from job!',
              from_email='tlfordjango@mail.ru',
              recipient_list=['kiromotossindzi@gmail.com', 'kiromoto@tut.by', ],
              )
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        scheduler.add_job(my_job,'cron', day_of_week="fri", hour="8-10", minute="*/5", # То же, что и интервал, но задача тригера таким образом более понятна django
                          id="my_job",   # уникальный айди
                          max_instances=1,
                          replace_existing=True,)
